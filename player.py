...
class Player:
    ...
    def continous_self_play(self,share_storage,replay_buffer,test_mode=False):
        ...
        while not ray.get(share_storage.get_info.remote("terminate")):
            ...
            if test_mode:   # Take the best action
                self.paly_game(False,replay_buffer)
        ...
        with torch.no_grad():
            while not done :
                ...
                if info["ale.lives"] != live:
                    ...
                    self.game.step(1)
                    fake_done=True
                game_history.save_transition(np.array(obs),action_index,reward,np.array(obs_),fake_done)
    ...
    def choose_action(self,obs):
        if random.random()>self.epsilon:
            obs_input=torch.FloatTensor(obs).permute(2,0,1).unsqueeze(0)
